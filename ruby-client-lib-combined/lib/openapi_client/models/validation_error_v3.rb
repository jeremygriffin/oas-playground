  ...
  class ValidationErrorV3
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ValidationErrorV3` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ValidationErrorV3`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'invalid_fields')
        if (value = attributes[:'invalid_fields']).is_a?(Hash)
          self.invalid_fields = value
        end
      else
        self.invalid_fields = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @invalid_fields.nil?
        invalid_properties.push('invalid value for "invalid_fields", invalid_fields cannot be nil.')
      end
