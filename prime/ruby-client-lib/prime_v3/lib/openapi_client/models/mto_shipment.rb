  ...
  class MTOShipment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::MTOShipment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::MTOShipment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [mto_service_items, id, move_task_order_id, approved_date, requested_pickup_date, requested_delivery_date, scheduled_pickup_date, actual_pickup_date, first_available_delivery_date, required_delivery_date, scheduled_delivery_date, actual_delivery_date, prime_estimated_weight, prime_estimated_weight_recorded_date, prime_actual_weight, nts_recorded_weight, customer_remarks, counselor_remarks, agents, sit_extensions, reweigh, pickup_address, destination_address, destination_type, secondary_pickup_address, secondary_delivery_address, storage_facility, shipment_type, diversion, diversion_reason, status, ppm_shipment, delivery_address_update, e_tag, created_at, updated_at, point_of_contact, origin_sit_auth_end_date, destination_sit_auth_end_date].hash
    end
