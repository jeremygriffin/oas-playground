  ...
  class MTOServiceItemV2
    ...
    def self.openapi_types
      {
        ...
        :'status' => :'MTOServiceItemStatusV2',
        :'rejection_reason' => :'String',
        :'model_type' => :'MTOServiceItemModelTypeV2',
        :'service_request_documents' => :'Array<ServiceRequestDocumentV2>',
        :'e_tag' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::MTOServiceItemV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::MTOServiceItemV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
