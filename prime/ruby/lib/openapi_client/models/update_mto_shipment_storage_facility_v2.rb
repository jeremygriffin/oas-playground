...
module OpenapiClient
  class UpdateMTOShipmentStorageFacilityV2
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UpdateMTOShipmentStorageFacilityV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UpdateMTOShipmentStorageFacilityV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'facility_name')
        self.facility_name = attributes[:'facility_name']
      end
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^[2-9]\d{2}-\d{3}-\d{4}$/)
    ...
    def hash
      [id, facility_name, address, lot_number, phone, email, e_tag].hash
    end
