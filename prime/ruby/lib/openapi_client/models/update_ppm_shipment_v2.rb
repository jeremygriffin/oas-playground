  ...
  class UpdatePPMShipmentV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UpdatePPMShipmentV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UpdatePPMShipmentV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [expected_departure_date, sit_expected, sit_location, sit_estimated_weight, sit_estimated_entry_date, sit_estimated_departure_date, estimated_weight, has_pro_gear, pro_gear_weight, spouse_pro_gear_weight].hash
    end
