  ...
  class UpdateMTOShipment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UpdateMTOShipment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UpdateMTOShipment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@actual_pro_gear_weight.nil? && @actual_pro_gear_weight < 1
        invalid_properties.push('invalid value for "actual_pro_gear_weight", must be greater than or equal to 1.')
      end

      if !@actual_spouse_pro_gear_weight.nil? && @actual_spouse_pro_gear_weight < 1
        invalid_properties.push('invalid value for "actual_spouse_pro_gear_weight", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@actual_pro_gear_weight.nil? && @actual_pro_gear_weight < 1
      return false if !@actual_spouse_pro_gear_weight.nil? && @actual_spouse_pro_gear_weight < 1
    ...
    # @param [Object] actual_pro_gear_weight Value to be assigned
    def actual_pro_gear_weight=(actual_pro_gear_weight)
      if !actual_pro_gear_weight.nil? && actual_pro_gear_weight < 1
        fail ArgumentError, 'invalid value for "actual_pro_gear_weight", must be greater than or equal to 1.'
      end

      @actual_pro_gear_weight = actual_pro_gear_weight
    end

    # Custom attribute writer method with validation
    # @param [Object] actual_spouse_pro_gear_weight Value to be assigned
    def actual_spouse_pro_gear_weight=(actual_spouse_pro_gear_weight)
      if !actual_spouse_pro_gear_weight.nil? && actual_spouse_pro_gear_weight < 1
        fail ArgumentError, 'invalid value for "actual_spouse_pro_gear_weight", must be greater than or equal to 1.'
      end

      @actual_spouse_pro_gear_weight = actual_spouse_pro_gear_weight
    end
    ...
    def hash
      [actual_pro_gear_weight, actual_spouse_pro_gear_weight, scheduled_pickup_date, actual_pickup_date, first_available_delivery_date, scheduled_delivery_date, actual_delivery_date, prime_estimated_weight, prime_actual_weight, nts_recorded_weight, pickup_address, destination_address, destination_type, secondary_pickup_address, secondary_delivery_address, storage_facility, shipment_type, diversion, point_of_contact, counselor_remarks, ppm_shipment].hash
    end
